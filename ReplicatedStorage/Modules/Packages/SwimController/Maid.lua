...
function Janitor:_Add(Object: any, MethodName: StringOrTrue?, Index: any?): any
	if Index then
		self:Remove(Index)
...
function Janitor:AddPromise(PromiseObject)
	if FoundPromiseLibrary then
		...
		if PromiseObject:getStatus() == Promise.Status.Started then
			...
			local NewPromise = self:_Add(Promise.new(function(Resolve, _, OnCancel)
...
function Janitor:LinkToInstance(Object: Instance, AllowMultiple: boolean?): RbxScriptConnection
	...
	Object = nil :: any
	return self:_Add(ManualDisconnect, "Disconnect", IndexToUse)
end
...
function Janitor:LinkToInstances(...: Instance)
	...
	for _, Object in ipairs({...}) do
		ManualCleanup:_Add(self:LinkToInstance(Object, true), "Disconnect")
	end
